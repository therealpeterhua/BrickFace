(function() {
  ...
  FaceDetect.prototype.handleDetectedFaces = function(faces) {
    // this.drawFaceBox(faces);

    this.vCtx.fillRect(20, 20, 50, 50);
    this.vCtx.fillRect(40, 40, 50, 50);
  ...
  FaceDetect.prototype.drawFaceBox = function(facesDetected) {
    var canvasCoords = [];
    facesDetected.forEach( function(face) {
      var scaled = this.scaleToCanvas(face);
      canvasCoords.push( scaled )
    }.bind(this));

    canvasCoords.forEach( function(coords) {
      this.vCtx.fillRect[ coords[0], coords[1], coords[2], coords[3] ];
    }.bind(this));
  };
